(() => {
  ...
  function renderTrueFalseDashboard(session, live) {
    ...
    return `<section class="content-grid"><article class="panel full ${live ? "panel-primary" : ""}">${panelHeading("Phân bố lựa chọn Đúng / Sai", live ? "Theo dõi trực tiếp" : "Đáp án đúng được đánh dấu màu xanh lá")}<div class="tf-grid" data-ui-scroll="session-${session.id}-true-false-questions">${questions.map((item, index) => {
      ...
      return `<button type="button" class="tf-card ${index === selectedQuestion ? "active" : ""}" data-question="${index}"><span class="tf-card-heading"><b>Câu ${index + 1}</b>${correctAnswer ? `<em>Đáp án: ${escapeHtml(correctAnswer)}</em>` : ""}</span><div class="${trueIsCorrect ? "correct-choice" : ""}"><strong>${formatNumber.format(trueCount)}</strong><small>Đúng · ${score(trueCount / total * 100)}%${trueIsCorrect ? '<b>Đáp án đúng</b>' : ""}</small></div><div class="${falseIsCorrect ? "correct-choice" : ""}"><strong>${formatNumber.format(falseCount)}</strong><small>Sai · ${score(falseCount / total * 100)}%${falseIsCorrect ? '<b>Đáp án đúng</b>' : ""}</small></div></button>`;
