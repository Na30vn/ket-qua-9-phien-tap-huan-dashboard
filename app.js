(() => {
  ...
  function renderOpenDashboard(session, live) {
    ...
    return `<section class="content-grid open-dashboard">${renderPromptCard(session, false)}${live ? "" : `<article class="panel full reference-panel">${panelHeading("Gợi ý / đáp án tham chiếu", "Dùng để đối chiếu và trao đổi tại lớp")}${reference.length ? `<ol class="reference-list">${reference.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ol>` : renderInlineEmpty("Chưa có nội dung tham chiếu.")}</article>`}<article class="panel full ${live ? "panel-primary" : ""}"><div class="panel-heading panel-heading-actions"><div><p class="panel-kicker">PHẢN HỒI HỌC VIÊN</p><h3>${live ? "10 phản hồi đầu tiên" : `Danh sách câu trả lời (${filtered.length}/${responses.length})`}</h3></div>${live ? '<span>Nội dung phản hồi được hiển thị ẩn danh</span>' : `<label class="search-box"><span class="sr-only">Tìm trong câu trả lời</span><input id="response-search" type="search" placeholder="Tìm trong nội dung phản hồi…" value="${escapeHtml(responseSearch)}"></label>`}</div><p class="privacy-note">Chỉ hiển thị nội dung phản hồi ẩn danh. Họ tên không được đưa lên API; đơn vị chỉ xuất hiện dưới dạng số liệu tổng hợp.</p>${filtered.length ? `<div class="response-list open-response-list">${filtered.map((text, index) => `<article class="open-response-card"><header><span>Phản hồi ${index + 1}</span></header><p>${escapeHtml(text)}</p></article>`).join("")}</div>` : renderInlineEmpty(responses.length ? "Không tìm thấy phản hồi phù hợp." : "Chưa có phản hồi. Dashboard sẽ tự cập nhật khi có dữ liệu.")}</article></section>`;
  }

  function renderPromptCard(session, collapsible) {
    ...
    if (collapsible) return `<details class="panel full prompt-card" open><summary><span>${escapeHtml(prompt.label || "ĐỀ BÀI")}</span><strong>${escapeHtml(prompt.title || "")}</strong><small>Thu gọn / mở rộng</small></summary><div class="prompt-body">${body}</div></details>`;
    return `<article class="panel full prompt-card"><div class="prompt-heading"><span>${escapeHtml(prompt.label || "ĐỀ BÀI")}</span><h3>${escapeHtml(prompt.title || "")}</h3></div><div class="prompt-body">${body}</div></article>`;
  }
