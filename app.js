(() => {
  ...
  function renderUnitParticipation(session) {
    ...
    return `<section class="content-grid unit-participation-section"><article class="panel full missing-units-panel ${missing.length ? "has-missing-units" : ""}"><div class="panel-heading"><div><p class="panel-kicker">THỐNG KÊ ĐƠN VỊ CHƯA THAM GIA</p><h3>${missing.length ? `${formatNumber.format(missing.length)} đơn vị chưa có bài` : "Tất cả đơn vị đã tham gia"}</h3></div><span class="type-pill">${formatNumber.format(participating)}/${formatNumber.format(total)} đơn vị đã tham gia</span></div>${unmapped ? `<p class="privacy-note" style="margin-top:0;">Có ${formatNumber.format(unmapped)} phản hồi mang tên đơn vị chưa khớp danh mục chuẩn.</p>` : ""}${missing.length ? `<details class="missing-units missing-units-expanded" data-ui-state="session-${session.id}-missing-units" open><summary><span class="missing-open">Thu gọn danh sách cần đôn đốc</span><span class="missing-closed">Xem danh sách cần đôn đốc</span></summary><ol class="missing-units-grid">${missing.map((unit, idx) => `<li><span class="unit-num">${idx + 1}.</span><span class="unit-name-text">${escapeHtml(unit)}</span></li>`).join("")}</ol></details>` : '<span class="all-units-badge">Đã đủ đơn vị trong danh mục</span>'}</article></section>`;
  }
