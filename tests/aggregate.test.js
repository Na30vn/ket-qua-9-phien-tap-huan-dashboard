...
assert.match(data.sessions[1].prompt.title, /Sắp xếp các hoạt động/);
assert.equal(data.sessions[2].referenceAnswer.length, 3);
assert.match(data.sessions[2].prompt.question, /Cần bổ sung gì/);
assert.equal(data.sessions[2].liveResponses.length, 1);
assert.equal(data.sessions[2].participatingUnits, 1);
assert.equal(data.sessions[5].questions[0].options[0].label, "Sai");
assert.equal(data.sessions[5].questions[0].explanations[0].selectedAnswer, "Sai");
assert.equal(data.sessions[5].questions[0].explanations[0].text, "Giải thích 1");
